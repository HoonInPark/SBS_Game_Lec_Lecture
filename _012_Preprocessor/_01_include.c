...
// 전처리기는 stdio.h의 모든 내용을 가져와서 아래 include문을 대체한다.
//#include <stdio.h>
...
/* 빌드 절차(소프트웨어를 완성하는 것을 건축에 비유하여 빌드라고 한다)
소스코드를 작성하고 이것이 exe파일로 만들어지는 절차가 있다.
1) 소스코드 작성(*.c)
1.5) 전처리 단계(소스코드에서 #이 붙은 키워드는 전처리기에 의해 치환된다)
2) 컴파일(소스코드를 기계어로 전환) *.obj 결과물
3) 링크(관련 라이브러리(lib, dll)를 컴파일 코드에 추가)
4) *.exe(바이너리 파일)나 *.lib, *.dll이 결과물
5) 실행 시 exe 는 메모리에 올라가서 프로세스가 되어 CPU에 의해 동작됨.

*.pdb는 심볼파일이라고 디버깅에 필요한 정보가 들어있다.
*/